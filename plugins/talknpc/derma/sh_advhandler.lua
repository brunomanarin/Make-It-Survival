...
PLUGIN.SpecialCall =
{
		...
		["quest_prensado"] = { -- QUEST EXAMPLE.
			sv = function( client, data ) 
				if client:HasQuest( "wendell09" ) then
					-- questPLUGIN = from the "quests" plugin.
					local pqst_dat = client:GetQuest( "wendell09" ) -- get player quest data
					if client:CanCompleteQuest( "wendell09", pqst_dat ) then -- If see player can complete quest
						client:GiveQuestReward( "wendell09", pqst_dat ) -- Give quest reward
						client:RemoveQuest( "wendell09" ) -- and remove player quest.
						data.done = true -- send client data.done. It will generate you're done text.
					else
						data.done = false
					end
					...
					-- set quest and get quest.
					data.gotquest = true -- Just got a quest!
					local d_qst = questPLUGIN:GetQuest( "wendell09" )
					client:AddQuest( "wendell09", d_qst:GenerateData( client ) ) -- Give a quest that has uniqueid 'wendell09' and generates random data for quest.
					-- Quest data generating function is in sh_quests.lua file.
				end
				return data -- MUST RETURN DATA
			end,
			cl = function( client, panel, data )
				if data.gotquest then
					local d_qst = questPLUGIN:GetQuest( "wendell09" )
					local pqst_dat = LocalPlayer():GetQuest( "wendell09" ) -- get player quest data
					panel:AddChat( data.name, "Pega o dedinho pro papai?" )
					for k, v in pairs( pqst_dat ) do
						panel:AddCustomText( Format( d_qst.desc, unpack( { v, nut.item.list[k].name } ) ), "nut_ChatFont" )
					end
					panel.talking = false -- Get quest and end the converstaion.
				...
				if data.done then
					panel:AddChat( data.name, "Valeu! shiuiu")
				else
					panel:AddChat( data.name, "Sai daqui LGBT, cade minha perninha porra!")
				end
				panel.talking = false
			end,
		},
