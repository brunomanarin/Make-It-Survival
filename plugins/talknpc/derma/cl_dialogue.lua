...
function PANEL:SetEntity( entity )
	...
	---------------------
		...
		local bodygroupslider = self.scroll:Add("DLabel")
		bodygroupslider:SetText("Bodygroups")
		bodygroupslider:DockMargin(3, 3, 3, 3)
		bodygroupslider:Dock(TOP)
		bodygroupslider:SetTextColor(Color(60, 60, 60))
		self.bodygroups = {}
		for k, v in pairs(entity:GetBodyGroups()) do
			...
			-- end
			-- self.classes[k] = panel
			local DermaNumSlider = self.scroll:Add("DNumSlider")
			DermaNumSlider:Dock( TOP ) 
			DermaNumSlider:SetSize( 300, 100 )		 
			DermaNumSlider:SetText( v.name )	
			DermaNumSlider:SetMin( 0 )				 
			DermaNumSlider:SetMax( #v.submodels )
			DermaNumSlider:SetDefaultValue( v.num )			
			DermaNumSlider:SetDecimals( 0 )	
			self.bodygroups[k] = DermaNumSlider		 
		end
		...
		self.save.DoClick = function()
			if (IsValid(entity) and (self.nextSend or 0) < CurTime()) then
				...
				for k, v in pairs(self.bodygroups) do
					if v:IsValid() then
						bodyGroupData[v:GetText()] = v:GetTextArea():GetValue()
					end
				end
				...
				netstream.Start("nut_DialogueData", { entity, dialogue, factionData, classData, self.name:GetText(), self.desc:GetText(), self.model:GetText() or entity:GetModel(), bodyGroupData } )
				self:Close()
			end
		end
end
