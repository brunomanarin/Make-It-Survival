...
ITEM.width = 1
ITEM.height = 1
ITEM.desc = "Classic rebel attire, don't go wearing this nearby combine forces."
...
ITEM:hook("Equip", function(item)
	if (SERVER) then
		...
		if(verifyItem(item)) then
			client:ConCommand("say /me  coloca a vestimenta rebelde, ajustando as alças e fivelas.")
...
ITEM:hook("EquipUn", function(item)
	if (SERVER) then
		local client = item.player
		client:ConCommand("say /me  retira a vestimenta rebelde, dobrando-a com as mãos e apoiando-a em seus braços.")
