ITEM.name = "CWU shirt with short sleeves"
ITEM.model = Model("models/tnb/items/aphelion/shirt_citizen2.mdl")
ITEM.width = 1
ITEM.height = 1
ITEM.desc = "A shirt that represents your affiliattion with the CWU. Shows your arms a little bit more than the other one."
...
ITEM:hook("Equip", function(item)
	if (SERVER) then
		...
		if(verifyItem(item)) then
			client:ConCommand("say /me abotoa uma camisa e passa a mão em cima da logo CWU. Ajeita as mangas logo em seguida.")
...
ITEM:hook("drop", function(item)
	if (SERVER) then
		local client = item.player
		client:ConCommand("say /me retira uma camisa da CWU de manga curta e deixa-a cair no chão.")
		client:StopSound("clothes_on")
	end
end)
